...
const averageCostPrice = computed(() => {
  const usable = batches.value
    .filter(b => USABLE_STATUSES.has(b.status) && b.quantity_remaining > 0)
    .sort((a, b) => (a.batch_id > b.batch_id ? 1 : -1))

  if (usable.length > 0) {
    // FIFO: pick the oldest batch (lowest batch_id) that would be consumed first
    return usable[0]?.cost_price || currentProduct.value?.cost_price || 0
  }
